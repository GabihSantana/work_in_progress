...
BEGIN 
    ...
    SET @query1 = CONCAT(
		'CREATE TABLE IF NOT EXISTS ', nomeTabelaFinal, ' AS
			 SELECT 
			(SELECT COUNT(LINHAA) FROM ', tabela, ' WHERE LINHAA IN (', CIDs, ')) AS Ocorrencias_Alzheimer_LinhaA,
			(SELECT COUNT(LINHAB) FROM ', tabela, ' WHERE LINHAB IN (', CIDs, ')) AS Ocorrencias_Alzheimer_LinhaB,
			(SELECT COUNT(LINHAC) FROM ', tabela, ' WHERE LINHAC IN (', CIDs, ')) AS Ocorrencias_Alzheimer_LinhaC,
			(SELECT COUNT(LINHAD) FROM ', tabela, ' WHERE LINHAD IN (', CIDs, ')) AS Ocorrencias_Alzheimer_LinhaD,

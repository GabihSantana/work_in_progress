...
int main(int argc, char *argv[]) {
    ...
    char *basename_ptr = basename(table_name); // Extrai o nome do arquivo
    int len = strlen(basename_ptr);

    // Remove a extensão .csv se existir
    if (len > 4 && strcmp(basename_ptr + len - 4, ".csv") == 0) {
        basename_ptr[len - 4] = '\0';
    }
